...
test("builds Guheyo search URLs", () => {
  assert.equal(
    buildGuheyoSearchUrl("realforce", "newest"),
    "https://guheyo.com/search/sell?q=realforce",
  );
});
...
test("finds Mercari next page token URL", () => {
  assert.equal(
    ...
    "https://jp.mercari.com/search?keyword=realforce&order=desc&page_token=v1%3A1&sort=created_time&status=on_sale",
  );
});
...
test("forces Mercari pagination URLs to use the on-sale status", () => {
  assert.equal(
    ...
    "https://jp.mercari.com/search?keyword=realforce&page_token=v1%3A2&status=on_sale&sort=created_time&order=desc",
  );
});
